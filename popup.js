document.addEventListener('DOMContentLoaded', function() {
    const form = document.getElementById('configForm');
    const options = ['option1', 'option2', 'option3', 'option4', 'option5', 'option6'];
    const statsInfo = document.getElementById('statsInfo');
    const currentStatus = document.getElementById('currentStatus');

    function updateStats() {
        const checkedCount = options.filter(option => document.getElementById(option).checked).length;
        const filledInputs = options.filter(option => document.getElementById('minOdds' + option.slice(-1)).value.trim() !== '').length;
        statsInfo.textContent = `统计信息: 已选择 ${checkedCount} 项，共填写 ${filledInputs} 个输入框`;
    }

    // 加载保存的配置
    function loadSavedConfig() {
        const savedConfig = JSON.parse(localStorage.getItem('config')) || {};
        options.forEach(function(option) {
            const checkbox = document.getElementById(option);
            const minOddsInput = document.getElementById('minOdds' + option.slice(-1));

            if (savedConfig[option]) {
                checkbox.checked = savedConfig[option].checked;
                minOddsInput.value = savedConfig[option].minOdds;
            }
        });
        updateStats();
        currentStatus.textContent = '当前状态: 已加载保存的配置';
    }

    // 监听所有复选框和输入框的变化
    options.forEach(function(option) {
        const checkbox = document.getElementById(option);
        const minOddsInput = document.getElementById('minOdds' + option.slice(-1));

        checkbox.addEventListener('change', function() {
            updateStats();
            currentStatus.textContent = '当前状态: 未保存';
        });

        minOddsInput.addEventListener('input', function() {
            updateStats();
            currentStatus.textContent = '当前状态: 未保存';
        });
    });

    // 保存配置
    form.addEventListener('submit', function(e) {
        e.preventDefault();
        const data = {};

        options.forEach(function(option) {
            const checkbox = document.getElementById(option);
            const minOddsInput = document.getElementById('minOdds' + option.slice(-1));
            data[option] = {
                checked: checkbox.checked,
                minOdds: minOddsInput.value
            };
        });

        localStorage.setItem('config', JSON.stringify(data));
        console.log('配置已保存');
        currentStatus.textContent = '当前状态: 已保存';
        // 可以在这里添加保存成功的提示
    });

    // 初始加载保存的配置
    loadSavedConfig();

    // 初始更新统计信息
    updateStats();
});
